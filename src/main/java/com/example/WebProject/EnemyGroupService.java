...
public class EnemyGroupService {
    ...
    public List<EnemyGroupMember> createRandomGroup(int size, List<EnemyTemplate> enemies) {
        if(enemies.isEmpty()){
        ...
        for (int i = 0; i < size; i++) {
            ...
            EnemyTemplate randomEnemy = enemies.get(rand.nextInt(group.size()));
    ...
    public List<EnemyGroupMember> createRandomGroupWithAll(int size) {
        List<EnemyTemplate> allEnemies = enemyRepository.findAll();
    ...
    public List<EnemyGroupMember> createRandomGroupByBiomes(int size, List<String> biomes) {
        List<EnemyTemplate> filtered = enemyRepository.findAll().stream().filter(e -> biomes.contains(e.getBiom())).toList();
    ...
    public List<EnemyGroupMember> createRandomGroupBySpecies(int size, List<String> species) {
        List<EnemyTemplate> filtered = enemyRepository.findAll().stream().filter(e -> species.contains(e.getSpecies())).toList();
        return createRandomGroup(size, filtered);
    }
    ...
    public List<EnemyGroupMember> createRandomGroupByBiomesAndSpecies(int size, List<String> biomes, List<String> species) {
        List<EnemyTemplate> filtered = enemyRepository.findAll().stream()
